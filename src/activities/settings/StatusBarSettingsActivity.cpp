...
bool StatusBarSettingsActivity::handleTouch() {
  ...
  const int previewTop = renderer.getScreenHeight() - UITheme::getInstance().getStatusBarHeight() -
                         verticalPreviewPadding - verticalPreviewTextPadding - metrics.verticalSpacing;
  const Rect listBounds{screen.x, contentTop, screen.width, std::max(0, previewTop - contentTop)};
  const int listRows = std::max(1, listBounds.height / metrics.listRowHeight);
  const auto layout = TouchList::calculatePageLayout(selectedIndex, visibleItemCount, listRows);
  ...
  if (TouchList::isPreviousPageRow(layout, visibleRow)) {
    selectedIndex =
        TouchList::calculatePageLayout(std::max(0, layout.start - 1), visibleItemCount, listRows).start;
    requestUpdate();
    return true;
  }
  if (TouchList::isNextPageRow(layout, visibleRow)) {
    selectedIndex = std::min(visibleItemCount - 1, layout.start + layout.itemCount);
    requestUpdate();
    return true;
  }
...
void StatusBarSettingsActivity::render(RenderLock&&) {
  ...
  const Rect screen = UITheme::getInstance().getScreenSafeArea(renderer, false, false);
  TouchUi::drawHeaderWithBack(renderer, screen, tr(STR_CUSTOMISE_STATUS_BAR));
#else
...
#ifdef CROSSPOINT_BOARD_MURPHY_M4
      pageHeight - contentTop - UITheme::getInstance().getStatusBarHeight() - verticalPreviewPadding -
      verticalPreviewTextPadding - metrics.verticalSpacing;
#else
  ...
  GUI.drawList(
      ...
      [layout](int visibleRow) {
        ...
        return std::string(I18N.get(menuNames[TouchList::visibleRowToItemIndex(layout, visibleRow)]));
      },
      nullptr, nullptr,
      [layout](int visibleRow) -> std::string {
