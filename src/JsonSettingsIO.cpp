...
bool JsonSettingsIO::saveBookmarks(const std::vector<BookmarkEntry>& bookmarks, const char* path) {
  ...
  for (const auto& bookmark : bookmarks) {
    ...
    obj["spineIndex"] = bookmark.computedSpineIndex;
    obj["chapterPageCount"] = bookmark.computedChapterPageCount;
    obj["chapterProgress"] = bookmark.computedChapterProgress;
  }
...
bool JsonSettingsIO::loadBookmarks(std::vector<BookmarkEntry>& bookmarks, const char* json) {
  ...
  for (JsonObject obj : arr) {
    ...
    bookmark.computedSpineIndex = obj["spineIndex"] | static_cast<uint16_t>(0);
    bookmark.computedChapterPageCount = obj["chapterPageCount"] | static_cast<uint16_t>(0);
    bookmark.computedChapterProgress = obj["chapterProgress"] | static_cast<uint16_t>(0);
  }
