...
CssWritingMode CssParser::interpretWritingMode(std::string_view val) {
  val = stripTrailingImportant(trimCssWhitespace(val));
  if (iequalsAscii(val, "vertical-rl") || iequalsAscii(val, "tb-rl")) return CssWritingMode::VerticalRl;
  if (iequalsAscii(val, "vertical-lr") || iequalsAscii(val, "tb-lr")) return CssWritingMode::VerticalLr;
...
void CssParser::parseDeclarationIntoStyle(std::string_view decl, CssStyle& style) {
  ...
  if (iequalsAscii(name, "text-align")) {
    ...
    style.writingMode = interpretWritingMode(value);
    style.defined.writingMode = 1;
  } else if (iequalsAscii(name, "text-combine-upright") || iequalsAscii(name, "-webkit-text-combine-upright") ||
             iequalsAscii(name, "-webkit-text-combine") || iequalsAscii(name, "-epub-text-combine")) {
  ...
  forEachDelimitedToken(
      ...
      [&](std::string_view sel) {
        ...
        if (it != rulesBySelector_.end()) {
          ...
          rulesBySelector_.emplace(std::string(sel), filteredStyle);
        }
      });
}
...
CssWritingMode CssParser::preferredWritingMode() const {
  for (const char* key : {"html", "body"}) {
    const auto it = rulesBySelector_.find(std::string_view(key));
